        ...
        <div>
            ...
            {questions.map((question,open) => <div key={question._id}
                ...
                <div
                    ...
                    <span>{openAns === open ? <BiMinus className='text-2xl'/> : <BsPlus className='text-2xl'/>}</span>
                   
                </div>
                <p className={openAns === open ? 'mt-2 text-gray-900 h-auto duration-300 transition-all' :'hidden'}>{question.answer}</p>
            </div>)
}
        </div>
    );
